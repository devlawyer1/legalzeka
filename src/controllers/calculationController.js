...
const activateRuleVersion = handler(async (req, res) => { const data = await getCalculationEngine().ruleService.activate(req.params.ruleCode, uuid.parse(req.params.versionId)); await auditRule(req, 'RULE_VERSION_ACTIVATED', data); res.json({ success: true, data }); });
